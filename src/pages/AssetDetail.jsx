...
import { ArrowLeft, Calendar, Clock, PenTool, User, CheckCircle, AlertTriangle } from 'lucide-react';
import { useMachines } from '../context/MachineContext';
import CheckOutModal from '../components/CheckOutModal';
// We will create this simple Return Modal in Step 2
import ReturnModal from '../components/ReturnModal'; 

const AssetDetail = () => {
  const { id } = useParams(); // 1. Get the ID from the URL
  const navigate = useNavigate();
  const { machines, loading, checkOutMachine, returnMachine } = useMachines();
  
  // State for Modals
  const [isCheckOutOpen, setCheckOutOpen] = useState(false);
  const [isReturnOpen, setReturnOpen] = useState(false);

  // 2. Find the specific machine
  const machine = machines.find(m => m.id === id);

  // 3. Loading State (Prevents crash on refresh)
  ...
  return (
    <div className="max-w-5xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="flex flex-col md:flex-row">
          ...
          <div className="w-full md:w-2/3 p-8 flex flex-col justify-between">
            <div>
              <div className="flex justify-between items-start mb-4">
                <div>
                  ...
                  <p className="text-gray-500 font-medium mt-1">{machine.type} • {machine.specs?.serialNumber || 'SN-12345'}</p>
                </div>
                {isAssigned && (
            ...
            <div className="flex gap-4 mt-8 pt-8 border-t border-gray-100">
              ...
              <button className="px-6 py-3 border-2 border-gray-200 text-gray-600 rounded-xl font-bold hover:bg-gray-50 transition-all">
                Maintenance
              </button>
            </div>
          </div>
      ...
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-8">
        <h3 className="text-xl font-bold text-gray-900 mb-6">History Log</h3>
          ...
          {(machine.history || []).length > 0 ? (
            machine.history.map((log, index) => (
              <div key={index} className="flex gap-4 items-start pb-6 border-b border-gray-50 last:border-0 last:pb-0">
                ...
                <div>
                  <p className="font-bold text-gray-900">{log.type || 'Activity Logged'}</p>
                  <p className="text-gray-500 text-sm mt-1">{log.notes || 'No notes provided.'}</p>
                  <p className="text-xs text-gray-400 mt-2">{log.date}</p>
                </div>
              </div>
            ))
          ) : (
            <div className="text-center py-10 text-gray-400">No history available yet.</div>
          )}
        </div>
      </div>
